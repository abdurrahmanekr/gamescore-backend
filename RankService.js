...
class RankService {
    ...
    // Haftalık dağıtım işini yapan işlem
    async weeklyDistribution() {
        ...
        // kişi başı dağılamıyorsa kimseye adil bir şekilde kimseye dağılmasına gerek yok
        if (leftMoney > 0) {
        ...
        console.log('Haftalık para dağıtma bitti');

        await delValue(['user_money']);
        await delValue(['user_today_money']);

        console.log('Score Tablosu sıfırlandı');
    }
    ...
    async dailyRankCalc() {
        ...
        console.log('Günlük score kaydetme tamamlandı')
    }
}
